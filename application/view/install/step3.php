		...
		$('input[name=db_password]').bind('keyup focus', function() {
			...
			$.post(base_url + 'ajax/confirm_database', {
					...
					if (data.success == 'true') {
						 $('#confirm_db').html(data.message).removeClass('alert-message block-message error').addClass('alert-message block-message success');
					} else {
						$('#confirm_db').html(data.message).removeClass('alert-message block-message success').addClass('alert-message block-message error');
					}
				}, 'json'
			);
		});
