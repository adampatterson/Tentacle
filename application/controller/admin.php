...
//		    url::redirect('admin/agree');
    ...
    public function content_update_page( $page_id ) {
        ...
        $get_page = $this->content_model()->type( 'page' )->get( $page_id );
    ...
    public function content_order_page() {
        ...
        $pages = $this->content_model()->type( 'page' )->get_by_parent_id( 0 );
    ...
    public function content_manage_pages( $status = '' ) {
        ...
        if ( $status ):
            $page_hierarchy = $this->content_model()->type( 'page' )->get_by_status( $status );
        else:
            $pages = $this->content_model()->type( 'page' )->get();

            $page_hierarchy = $this->content_model()->type( 'page' )->get_page_children( 0, $pages );
        endif;
    ...
    public function content_update_post( $post_id ) {
        ...
        $get_post = $this->content_model()->type( 'post' )->get( $post_id );
    ...
    public function content_manage_posts( $status = '' ) {
        ...
        if ( $status ) {
            $posts = $this->content_model()->type( 'post' )->get_by_status( $status );
        } else {
    ...
    public function settings_general() {
        ...
        $page_array = $this->content_model()->type( 'page' )->get_page_children( 0, $pages, 0 );

        $page_object = (object) $page_array;

        $get_page_by_level = $this->content_model()->type( 'page' )->get_page_by_level( $page_object, 0 );
